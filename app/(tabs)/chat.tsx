...
export default function ChatScreen() {
  ...
  const sendPersonaPriming = async () => {
    if (!personaData?.full_analysis) return;

    const primingMessage = `
  You are Kai, a personal concierge AI.
  Here is the user's profile:
  
  ${personaData.full_analysis}
  
  Use this information to personalize your responses.
  `;

    await messageToServer(primingMessage, 'text');
  };
  ...
  const joinChatRoom = async () => {
    if (!chatRoomJoined) {
      ...
      if (sessionId && socketRef.current) {
        ...
        // ✅ Inject persona data as a message for AI prompt context
        await sendPersonaPriming();
      } else {
        console.log('Cannot join chat room: sessionId not available yet.');
      }
    }
